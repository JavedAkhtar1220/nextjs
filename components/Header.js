    ...
    return (
        <header className="sticky top-0 z-50 grid grid-cols-2 bg-[#0199dc] shadow-md p-5 md:px-10">
             {/* Dynamic Logo */}
              ...
              src={"/logo-with-text.svg"} layout="fill" objectFit="contain" objectPosition="left" className="absolute cursor-pointer"/>
              </div>
             {/* Search Bar */}
            ...
            {session ? (
             <>
              ...
              ) : ( <button onClick={() => router.push('/auth/signin')} className="text-white">Sign in</button>)
              }
        </header>
    )
}
